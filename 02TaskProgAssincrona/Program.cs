...
{
    Console.WriteLine("-Fervendo a água");
    await Task.Delay(2000);//passa pra próxima execução (provavelmente á para a linha 25 até a execução 62)
    Console.WriteLine("-Coando o café");
    await Task.Delay(2500);//passa pra próxima execução
    Console.WriteLine("-Adoçando o café");
    return new Cafe();
}
...
{
    Console.WriteLine("*Partir o pão");
    await Task.Delay(2000);//passa pra próxima execução (provavelmente á para a linha 28 até a execução 54)
    Console.WriteLine("*Passar manteiga no pão");
    await Task.Delay(2000);//passa pra próxima execução
    Console.WriteLine("*Tostar o pão");
    return new Pao();
}
