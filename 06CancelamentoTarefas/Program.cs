...
{
    private static CancellationTokenSource? cancellationTokenSource; //Somente para cancelamento Imediado, o com time foi criado dentro do using 
    //permite ponto de entrada assincrono
    static async Task Main(string[] args)
    {
        var stopwatch = new Stopwatch();
        stopwatch.Start(); //medir o tempo

        try
        {
            /* Cancelamento de Imediato -----------------------------------------------------------
            cancellationTokenSource = new CancellationTokenSource();
            cancellationTokenSource?.Cancel(); //cancelamento

            var result = await OperacaoLongaDuracaocancelavel(100, cancellationTokenSource.Token);
            Console.WriteLine(result);
            */
            /* Cancelamento com Tempo -------------------------------------------------------------
            Console.WriteLine("Cancelamento automatico após 3,5 segundos");
    ...
    public static async Task ExecutaCancelamentoManual() // de acordo com o usuario 
    {
    ...
    private static Task<int> OperacaoLongaDuracaoCancelavel(int valor,
        CancellationToken cancellationToken = default) //passando o token de cancelamento 
    {
        Task<int> task = null;
    ...
    {
        using (var cancellationTokenSource = new CancellationTokenSource())//1º Metodo: CancellationTokenSource(tempo)
        {
            cancellationTokenSource.CancelAfter(tempo); // passando o 2º metodo
            try
            {
                var resultado = await OperacaoLongaDuracaoCancelavel(100,
                    cancellationTokenSource.Token); // chama a operação cancelavel 

                Console.WriteLine($"Resultado {resultado}");
            }
            catch (TaskCanceledException)
            {
                throw; //relançando a exceção para a linha 28
            }
        }
    }
