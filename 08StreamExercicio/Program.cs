...
static void CriarArquivo(string caminhoArquivo)
{
    try
    {
        //usado para garantir a execução eficiente e segura
...
static void GravarEmArquivo(string caminhoArquivo)
{
    ...
    try
    {
        ...
        { // o true especifica que o texto será gravado/anexado no final do arquivo, sem subestituir o texto anterior
...
{
    if(!File.Exists(caminhoArquivo)) //verifica se o arquivo não existe
    {
